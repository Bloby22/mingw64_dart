...
if (!$no_build) {
    make_path($outdir);
    # Mirror the compiler invocation from PKGBUILD build():
    # split into tokens, drop line-continuation backslashes, remove shell
    # quotes and expand shell variables used by the recipe.
    my ($compiler, $gxx_line) = $pkg =~ /((?:g\+\+|\$\{CXX\}))(.*?)\n\s*-lcurl/s
        or die "Could not find the compiler command in $pkgbuild\n";
    $compiler = 'g++' if $compiler eq '${CXX}';
    my @flags;
    for my $tok (split ' ', $gxx_line) {
        ...
        $tok =~ s/\$\{pkgver\}/$pkgver/g;
        $tok =~ s/\$\{(?:CPPFLAGS|CXXFLAGS|LDFLAGS)\}//g;
        next if $tok eq '';
        push @flags, $tok;
    }
    print "Compile: $compiler @flags -lcurl\n";
